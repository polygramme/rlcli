...
- `--max-steps-off-policy K` (`rlcli train harbor` and `rlcli train rl`) runs the cookbook's async loop: rollout workers keep sampling while the trainer steps, and a group sampled more than K steps ago is requeued instead of trained on. Sandboxes and the trainer stop waiting for each other; with importance-sampling losses the leftover staleness is corrected per token. Default 0 (synchronous).
